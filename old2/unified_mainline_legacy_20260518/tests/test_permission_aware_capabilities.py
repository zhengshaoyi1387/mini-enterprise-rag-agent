...
def test_permission_aware_contracts_are_filtered_by_role() -> None:
    ...
    update_contract = admin_calendar["actions"]["update"]["in"]
    assert {"event_id", "title", "time", "location"} <= set(update_contract)
    assert "fields" not in update_contract
...
def test_employee_calendar_delete_task_is_blocked_during_plan_validation(tmp_path: Path) -> None:
    nodes = AgenticRAGNodes(make_settings(tmp_path), llm=RecordingLLM([]))
    state = create_initial_state("查一下明天会议并删除", role="employee")
    state["raw_plan"] = {
        "message_type": "business_question",
        "context_usage": "none",
        "intent": "daily_tool",
        "route": "tool",
        "standalone_query": "查一下明天会议并删除",
        "topic": "calendar_write",
        "selected_tool": "manage_company_calendar",
        "selected_action": "query",
        "tool_input": {"action": "query"},
    ...
    assert state["plan_validation"]["normalization_reason"] == "task action not visible"
