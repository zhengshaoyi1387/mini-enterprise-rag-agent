...
def evaluate_tool_rule(case: dict[str, Any], result: dict[str, Any], trace: dict[str, Any], latency_ms: float) -> tuple[dict[str, Any], list[str]]:
    ...
    Design principle:
    - Tool selection, action, sequence, args, permission, write safety and write verification are hard metrics.
    - Natural-language wording is soft by default. `must_contain` is reported, but it only fails the case when
      the case explicitly sets `strict_answer_contains=true` or `must_contain_required=true`.
    - Permission/refusal cases should not be forced to call a tool. A safe early refusal is acceptable.
    """
    ...
    if should_refuse and not require_tool_when_refused:
        # A refusal can happen before the tool layer. Do not penalize the runner for not calling a forbidden tool.
        route_ok = True
        tool_ok = True
        action_ok = True
        sequence_ok = True
    ...
    # Verify writes only when the case explicitly expects a real write.
    # Missing-slot / permission-refusal cases may still have expected_action=create/update/delete, but they should not fail here.
...
def evaluate_e2e_rule(case: dict[str, Any], result: dict[str, Any], trace: dict[str, Any], latency_ms: float) -> tuple[dict[str, Any], list[str]]:
    ...
    E2E pass/fail focuses on task outcome, grounding and safety. Answer wording is soft by default.
    Use `strict_answer_contains=true` on a case only when an exact token is truly required.
    """
...
def evaluate_case(
    ...
    if should_run_llm_judge(args, case, failures):
        ...
        if args.judge == "llm":
            ...
            llm_task_zero
            and not llm_task_zero_ok
            and not expected.get("llm_task_success_soft")
        ):
            failures.append("llm judge task_success=0")
...
def normalize_eval_text(value: Any) -> str:
    ...
    text = re.sub(r"(\d{4})年0?(\d{1,2})月0?(\d{1,2})日", lambda m: f"{m.group(1)}-{int(m.group(2)):02d}-{int(m.group(3)):02d}", text)
    text = re.sub(r"(\d{4})/0?(\d{1,2})/0?(\d{1,2})", lambda m: f"{m.group(1)}-{int(m.group(2)):02d}-{int(m.group(3)):02d}", text)
    replacements = {
        "—": "-",
        "–": "-",
        "－": "-",
...
def must_have_args_match(expected_args: dict[str, Any], calls: list[dict[str, Any]], task_results: list[dict[str, Any]]) -> bool:
    ...
    for key, expected in expected_args.items():
        ...
        for payload in payloads:
            values = nested_values_for_key(payload, key)
            if any(value_matches(value, expected) for value in values):
                matched = True
                break
        if not matched:
            return False
...
def forbidden_args_absent(forbidden_args: dict[str, Any], calls: list[dict[str, Any]], task_results: list[dict[str, Any]]) -> bool:
    ...
    for key, forbidden in forbidden_args.items():
        for payload in payloads:
            values = nested_values_for_key(payload, key)
            if any(value_matches(value, forbidden) for value in values):
                return False
    return True
...
def expected_refusal_or_clarification_accepts_llm_task_zero(expected: dict[str, Any], llm_scores: dict[str, Any]) -> bool:
    """LLM judge task_success=0 is valid when non-execution is the expected outcome."""
