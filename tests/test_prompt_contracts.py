...
def test_finalize_prompt_requires_weekday_offsets_from_datetime_result(tmp_path: Path) -> None:
    ...
    nodes._finalize_tool_input_with_llm(
        state=state,
        tool_name="manage_company_calendar",
        payload={"action": "create", "title": "会议", "time": "08:00", "date": "next_tuesday"},
        datetime_result={
    ...
    user_prompt = llm.calls[0][-1][1]
    assert "下周二" in user_prompt
    assert "next_week.start_date 是周一" in user_prompt
    assert "周二 = start_date + 1 天" in user_prompt
